...
void dsntp_config_set_defaults(dsntp_config_t *cfg) {
    ...
    strncpy(cfg->server_endpoint, "http://127.0.0.1:8080", sizeof(cfg->server_endpoint) - 1);
}
...
dsntp_err_t dsntp_config_load(dsntp_config_t *cfg, const char *path) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        else if (strcmp(key, "server_endpoint") == 0) strncpy(cfg->server_endpoint, val, sizeof(cfg->server_endpoint) - 1);
