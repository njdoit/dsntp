/*
 * fsm.h — finite state machine (FR-FSM / DRS §6); PeerFault != LocalFault
 */
#ifndef DSNTP_FSM_H
#define DSNTP_FSM_H
...
typedef enum dsntp_event {
    ...
    DSNTP_EV_PEER_FAULT_SUSPECT,  /* must not change local state */
    DSNTP_EV_PEER_FAULT_CONFIRM,  /* must not change local state; emit FAULT_NOTIFY */
